...
PROTO_REGEX = re.compile(r'\(Proto(?:\s*([a-z0-9.]+))?\)|\(Possible Proto\)|\(Prototype(?:\s*(.*?))?\)', re.IGNORECASE)
SAMPLE_REGEX = re.compile(r'\(Sample(?:\s*([a-z0-9.]+))?\)', re.IGNORECASE)
DEMO_REGEX = re.compile(r'\(Demo(?:\s*([a-z0-9.]+))?\)|\((Multiplayer|Singleplayer|Labeled) Demo\)|\(Trial(?:\s*([a-z0-9.]+))?\)|\(Tech Demo(?:,)?.*?\)|\((?:GameCube\s*)?Preview\)', re.IGNORECASE)
